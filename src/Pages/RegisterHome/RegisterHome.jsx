...
const RegisterHome = () => {
  ...
  const onSubmit = async (data) => {
    ...
    if (id === null) {
      try {
        ...
        const response = await axios.post('http://api.gamaldigital.com/home', data, { headers })
      ...
      try {
        ...
        const response = await axios.put(`http://api.gamaldigital.com/home/${id}`, data, { headers })
        if (response.status === 201) {
          toast.success(response.data.message)
          clearStorage()
          window.location.href = '/admin'
  ...
  useEffect(() => {
    const fetchHome = async () => {
      try {
        const response = await axios.get(`http://api.gamaldigital.com/home/${id}`)

...
const Login = () => {
  ...
  const onSubmit = async (data) => {
    if (code !== '') {
      if (editId === null) {
        ...
        try {
          const response = await axios.post('https://api.gamaldigital.com/users', {
            email: data.email,
      ...
      if (editId === null) {
        ...
        try {
          const response = await axios.post('https://api.gamaldigital.com/users/login', {
            email: data.email,
            password: data.password
          })
          if (response.status === 200) {
        ...
        try {
          ...
          const response = await axios.put(`https://api.gamaldigital.com/users/${editId}`, data, { headers })
  ...
  const deleteUser = async (id) => {
    try {
      ...
      const response = await axios.delete(`https://api.gamaldigital.com/users/${id}`, { headers })
      if (response.status === 200) {
        toast.success(response.data.message)
        window.location.href = '/adminusers'
      } else {

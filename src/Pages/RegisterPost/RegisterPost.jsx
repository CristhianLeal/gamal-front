...
const RegisterPost = () => {
  ...
  const onSubmit = async (data) => {
    ...
    if (id === null) {
      try {
        ...
        check()
        const response = await axios.post('https://gamaldigital.com:8080/posts', data, { headers })
      ...
      try {
        ...
        const response = await axios.put(`https://gamaldigital.com:8080/posts/${id}`, data, { headers })
        if (response.status === 201) {
          toast.success(response.data.message)
          clearStorage()
          window.location.href = '/adminposts'
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        const response = await axios.get(`https://gamaldigital.com:8080/posts/${id}`)
